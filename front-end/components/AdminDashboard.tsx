...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
	...
	const rescheduleShiftsCount = shifts.filter(
		(shift) => shift.rescheduled
	).length;
	const cancelledShiftCount = shifts.filter(
	...
	const totalValidJobs = validMonthlyData.reduce(
		(sum, data) => sum + data.jobs,
		0
	);
	const averageJobsPerMonth =
		validMonthlyData.length > 0
			? totalValidJobs / validMonthlyData.length
			: 0;
		...
		<div className="p-8">
			...
			{validMonthlyData.length > 0 ? (
				...
				<div className="mb-6 p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded">
					...
					No monthly data available. The chart will appear when data
					is present.
				</div>
			)}
