...
const ManageAdmins = () => {
	...
	const [adminToTerminate, setAdminToTerminate] = useState<Admin | null>(
		null
	);
	const [creating, setCreating] = useState(false);
	const [terminating, setTerminating] = useState(false);

	const fetchWorkerDetails = useCallback(async (adminId: string) => {
		try {
			const response = await fetch(
				`http://localhost:8080/workers?supervisorId=${adminId}`
			);
			...
			console.error(
				`Error fetching worker details for admin ${adminId}:`,
				error
			);
		}
	}, []);
	...
	const fetchAdmins = useCallback(async () => {
		try {
			...
			const nonRootAdmins = data.filter((admin) => !admin.root);
			setAdmins(data);

			setWorkerDetails({});

			nonRootAdmins.forEach((admin) => {
	...
	const getAvailableSupervisors = (excludeId?: string) => {
		return admins.filter((admin) => !admin.root && admin.id !== excludeId);
	};
	...
	const handleTerminateAdmin = async () => {
		...
		try {
			const response = await fetch(
				`http://localhost:8080/admins/${adminToTerminate.id}/terminate`,
				{
					method: 'DELETE',
					headers: {
						'Content-Type': 'application/json',
							...
							<div className="space-y-2">
								...
								<div>
									<p className="font-medium text-sm sm:text-base">
										Workers ({admin.workers.length}):
									</p>
									{workerDetails[admin.id] ? (
										<ul className="list-disc list-inside">
											{workerDetails[admin.id].map(
												(worker) => (
													<li
														key={worker.id}
			...
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							To terminate {adminToTerminate?.name}, please select
							a new supervisor for their workers:
						</DialogDescription>
					</DialogHeader>
					<div className="py-4">
