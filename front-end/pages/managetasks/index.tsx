...
const ManageTasks: React.FC = () => {
	...
	const fetchShifts = useCallback(async () => {
		try {
			...
			const supervisorId = userDetails
			...
			if (supervisorId === 3) {
				...
				const uniqueWorkerIds = Array.from(
					new Set(response.data.flatMap((shift) => shift.workerIds))
				);
	...
	const filteredTasks = Array.isArray(shiftData)
		? shiftData.filter(
				(task) =>
					...
					(selectedWorker === null ||
						task.workerIds.includes(selectedWorker))
		  )
		: [];
	...
	const cancelShift = async (shiftId: number) => {
		try {
			const response = await axios.put(
				`http://localhost:8080/shifts/${shiftId}/update-status`,
				null,
				{
					params: {
						status: 'CANCELLED',
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
					{currentTasks.map((task) => (
						<TaskCard
							...
							cancelShift={() => cancelShift(task.id)}
							refetchAfterChange={handleTaskUpdate}
						/>
					))}
				</div>
			)}
